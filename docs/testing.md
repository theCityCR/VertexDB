...
| `planner_behavior_tests.cpp` | Access paths (incl. prefix `LIKE` / trigram intersect / regex residual full-scan / non-prefix LIKE), residuals, stats/cost, multi-index intersect/union, outer/`CROSS` join plans, `EXPLAIN`, indexed UPDATE/DELETE `WHERE` |
| `transaction_behavior_tests.cpp` | BEGIN/COMMIT/ROLLBACK (incl. invalid state), MVCC visibility, deferred WAL, txn-forbidden DDL/persistence |
...
The current suite contains 217 discovered GoogleTest cases across the files above.
