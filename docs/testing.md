...
| `persistence_behavior_tests.cpp` | Page store, snapshot v4/v5/v6/v7/v8, page-image/physical/torn WAL, durable WAL append/`reset`, durable `SAVE` publish (flush+fsync) |
...
| `constraint_behavior_tests.cpp` | Single- and multi-column `PRIMARY KEY` / `UNIQUE` / `NOT NULL` / simple `CHECK` (incl. OR/`=`/`<`/string literals) / `FOREIGN KEY` (`NO ACTION` / `CASCADE` / `SET NULL`) parse + DML rejection/cascade, auto HashEq indexes (`__pk_*` / `__uq_*` including composites), DROP INDEX refusal, catalog-no-database paths, save/load constraints |
...
The current suite contains 362 discovered GoogleTest cases across the files above.
