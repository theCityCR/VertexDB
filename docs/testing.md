...
| `parser_tests.cpp` | Tokenization, AST grammar (`INNER`/`LEFT`/`RIGHT`/`FULL`/`CROSS`, `LIKE`/`~`, literal `IN` lists vs `IN` subquery, nested `WITH` depth), `EXPLAIN` / `EXPLAIN ANALYZE`, `ParseError` source positions, CROSS-with-ON / bushy-join refusals |
...
The current suite contains 237 discovered GoogleTest cases across the files above.
