...
| `constraint_behavior_tests.cpp` | Single-column `PRIMARY KEY` / `UNIQUE` / `NOT NULL` / simple `CHECK` (incl. OR/`=`/`<`/string literals) / `FOREIGN KEY` (`NO ACTION`) parse + DML rejection, auto HashEq indexes (`__pk_*` / `__uq_*`), DROP INDEX refusal, catalog-no-database paths, save/load constraints |
...
The current suite contains 340 discovered GoogleTest cases across the files above.
